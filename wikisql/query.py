...
class Query:
    ...
    def __init__(self, sel_index, agg_index, columns, use_column_name, conditions=tuple()):
        self.sel_index = sel_index
        self.agg_index = agg_index
        self.conditions = list(conditions)
        self.columns = columns
        self.use_column_name = use_column_name
    ...
    def __repr__(self):
        ...
        if self.use_column_name:
            sel_col = self.columns[int(self.sel_index)]
        else:
            sel_col = 'col{}'.format(int(self.sel_index))

        if agg != '':
            rep = 'SELECT {agg}("{sel}") FROM Table_1'.format(
                agg = self.agg_ops[self.agg_index],
                sel = sel_col,
            )
        else:
            rep = 'SELECT "{sel}" FROM Table_1'.format(
                sel = sel_col,
            )
        if self.conditions:
            ...
            for i, o, v in self.conditions:
                #print("i: " + str(i) + " o: " + str(o) + " v: " + str(v) + " type of v: " + str(type(v)))
                if self.use_column_name:
                    cols = self.columns[int(i)]
                else:
                    cols = 'col{}'.format(i)
                if type(v) == int:
                    temp2 = '"{}" {} {}'.format(cols, self.cond_ops[o], v)
                else:
                    temp2 = '"{}" {} "{}"'.format(cols, self.cond_ops[o], v)
                temp3.append(temp2)
            temp = ' WHERE ' + ' AND '.join(temp3)
            rep += temp+";"
            #print("--- temp: " + str(temp))
        return rep
